...
bool PDFOpenerHandler::RegisterFileAssociation() {
  ...
  if (!WriteRegistryKey(L"HKCU", 
                         ...
                         L"",
                         L"ABdSukaPDFDocument")) {
    return false;
  }

  // Create the ABdSukaPDFDocument ProgID
  std::wstring progid_path = L"Software\\Classes\\ABdSukaPDFDocument";
  if (!WriteRegistryKey(L"HKCU", progid_path, L"", L"PDF Document - ABdSukaPDF")) {
    return false;
  }
...
bool PDFOpenerHandler::RegisterUriScheme() {
  ...
  if (!WriteRegistryKey(L"HKCU", url_scheme_key, L"", L"ABdSukaPDF Handler")) {
    return false;
  }
